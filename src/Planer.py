...
class Plan():
    def __init__(self, bot):
        self.current_time = 0
        self.average_collection_rate_per_s = 40 / 60 # not sure if this is correct
        self.bot = bot
        self.mineral_guess = None
        self.time = np.array([0])
        self.mineral_collection_rate = np.array([0])
        self.mining_workers = sum([w_manager.mineral_worker_tags.amount for w_manager in self.bot.w_managers])
        self.working_until = [0 for _ in range(self.bot.townhalls.amount)]
        self.empty_worker_space_total = sum([self.empty_worker_space(w_manager) for w_manager in self.bot.w_managers])
        self.mineral_guess = None
    ...
    def build_worker(self):
        for index, i in enumerate(self.working_until):
            if i < self.current_time and self.empty_worker_space_total > 0:
                ...
                self.update_mineral_rate(self.current_time + 11, self.mining_workers * self.average_collection_rate_per_s)
    ...
    def update_mineral_rate(self, t, m):
        ...
        self.mineral_collection_rate = np.append(self.mineral_collection_rate, m)
        #self.mineral_guess = interp1d(self.time, self.mineral_collection_rate, 'linear')

    def update_minerals_collected(self, time):
        x = np.array([50])
        y = np.array([0])
        for i in range(1,int(time)+1): # +1, so we can -1 later, and always have valid index
            x = np.append(x, x[i-1] + self.get_mineral_rate(i))
            y = np.append(y, i) # can be made faster
